...
/* If you declare any globals in php_mdbm.h uncomment this:
...
/* {{{ proto string confirm_mdbm_compiled(string arg)
   Return a string to confirm that the module is compiled in */
PHP_FUNCTION(confirm_mdbm_compiled)
{
	char *arg = NULL;
	size_t arg_len, len;
	zend_string *strg;
	...
	strg = strpprintf(0, "Congratulations! You have successfully modified ext/%.78s/config.m4. Module %.78s is now compiled into PHP.", "mdbm", arg);
...
PHP_MINFO_FUNCTION(mdbm)
{
	...
	php_info_print_table_header(2, "mdbm support", "enabled");
...
/* {{{ mdbm_functions[]
 *
 * Every user visible function must have an entry in mdbm_functions[].
 */
const zend_function_entry mdbm_functions[] = {
	PHP_FE(confirm_mdbm_compiled,	NULL)		/* For testing, remove later. */
